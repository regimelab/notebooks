...
def fractional_gaussian_covariance(n:int, hurst1:float, hurst2:float, rho:float, sigma1:float=1, sigma2:float=1):
    
    w = lambda h: rho * abs(h)**(hurst1 + hurst2)
...
def fractional_gaussian_covariance_series(size:int, hurst1:float, hurst2:float, corr:float, sigma1:float=1, sigma2:float=1):
    ...
    series[:size, :size] = toeplitz([rho(i, hurst1) for i in range(size)])   
    series[-size:, -size:] = toeplitz([rho(i, hurst2) for i in range(size)])
