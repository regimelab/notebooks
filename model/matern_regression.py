...
# Plot 2: 30 sampled forecast paths with consistent styling and no cropping on history

n_simulations = 30

fig2, ax2 = plt.subplots(figsize=(12, 6))

y_samples_30 = gpr.sample_y(X_forecast, n_samples=n_simulations, random_state=None)  # shape (n_forecast_steps, n_simulations)

# Plot full historical data (no cropping)
ax2.plot(np.arange(len(df)), y, plot_marker_observed, color=plot_color_observed, label='Observed YoY CPI Diff')
ax2.plot(np.arange(len(df)), y_pred, '-', color=plot_color_pred, linewidth=plot_linewidth, label='GP Fit (Historical)')

# Plot simulations for forecast period
for i in range(n_simulations):
    ax2.plot(all_indices[-n_samples:], y_samples_30[:, i], alpha=0.7, color=plot_color_observed,
             label=f'Simulation {i+1}' if i < 5 else None)

ax2.plot(all_indices[-n_samples:], gpr.predict(X_forecast), color=plot_color_pred, linewidth=plot_linewidth, label='GP Mean Forecast')

_, y_std_forecast_30 = gpr.predict(X_forecast, return_std=True)
ax2.fill_between(all_indices[-n_samples:],
                 gpr.predict(X_forecast) - 1.96 * y_std_forecast_30,
                 gpr.predict(X_forecast) + 1.96 * y_std_forecast_30,
                 color=plot_color_forecast, alpha=plot_alpha_forecast, label='95% CI')

# Set x-axis limits and ticks for full range
ax2.set_xlim(0, all_indices[-1])
ax2.set_xticks(all_indices)
ax2.set_xticklabels(date_labels, rotation=45, ha='right')

ax2.set_xlabel('Date')
ax2.set_ylabel('YoY ΔCPI')
ax2.set_title(f'{n_simulations} Sampled Forecast Paths with Full Historical Data')
